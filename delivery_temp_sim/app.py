...
st.markdown("""
    <style>
    /* グラフのコンテナを横スクロール可能にする */
    .stPlotlyChart {
        overflow-x: auto !important;
        -webkit-overflow-scrolling: touch;
    }
    /* グラフ自体の最小幅を固定してスマホでも潰れないようにする */
    .plot-container {
...
if data.get("list"):
    ...
    # 表示範囲を1.5日分(14データ分)に短縮
    for item in data["list"][:14]:
    ...
    # --- グラフ作成 ---
    fig = make_subplots(rows=2, cols=1, shared_xaxes=True, vertical_spacing=0.1, subplot_titles=("温度推移 (℃)", "天候詳細 (降水・風速)"))
    ...
    # インタラクションを制限（ズーム禁止・操作無効化）
    fig.update_layout(
        height=500,
        dragmode=False, # ドラッグによる範囲選択を禁止
        hovermode="x unified",
    ...
    # グラフの表示（configでメニューバーを消去）
    st.plotly_chart(fig, use_container_width=False, width=800, config={'displayModeBar': False, 'staticPlot': False})

    # --- アドバイス ---
    st.subheader("💡 稼働アドバイス")
    # スマホで見やすいよう直近4件程度を表示
    cols = st.columns(min(len(df), 2)) # 縦に並ぶよう調整
    for i in range(4):
        with st.container():
            st.write(f"**{df['日時'].iloc[i]}** : {df['体感温度'].iloc[i]} ℃")
